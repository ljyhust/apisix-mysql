...
function _M.query_plugin_configs_by_time(self, fetch_start_time, fetch_end_time)
    local nowStr = os.date("%Y-%m-%d %H:%M:%S", fetch_end_time)
    local last_ctime = "0000-00-00 00:00:00"
    if nil ~= fetch_start_time then
        last_ctime = os.date("%Y-%m-%d %H:%M:%S", fetch_start_time)
    ...
    local query_plugin_confis_sql = format(self.query_plugin_config_sql_tpl, last_ctime, nowStr)
    log.info("query plugin configs ", query_plugin_confis_sql)

    local db_cli = get_conn(self.db_config)

    local res, err, errcode, sqlstate = db_cli:query(query_plugin_confis_sql)
    if not res then
    ...
    if nil == next(res) then
        log.info("no new plugin configs info")
        return
    end

    local plugin_configs_list = res
    local plugin_config_map = new_tab(0, #plugin_configs_list)
    for i, plugin_conf in ipairs(plugin_configs_list) do
        if plugin_conf.plugins then
            plugin_conf.plugins = json.decode(plugin_conf.plugins)
        end

        plugin_config_map["p" .. plugin_conf.id] = plugin_conf
    end
